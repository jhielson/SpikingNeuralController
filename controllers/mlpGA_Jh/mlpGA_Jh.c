...
void split(){ 
    ...
    for (int j = 0; j < 4; j++) { // hidden layer
        for (int k = 0; k < 4; k++) { // input layer
            matrix1[5+j][k] = vectorWeights[4*j+k+56];
        }
    }
}
*/
...
void sense_compute_and_actuate(WbDeviceTag left_motor, WbDeviceTag right_motor) {
    ...
    // Position
    /////////////////////////////////////////////////////////////////////////
    WbNodeRef robot = wb_supervisor_node_get_from_def("EPUCK_1_MLP");
    ...
    */
    ...
    {      
      ...
      if(position[0] < 3.5) fitnessValue *= 0.6; // 2/3      
    }
