...
void sense_compute_and_actuate(WbDeviceTag left_motor, WbDeviceTag right_motor) {
    ...
    // Position
    /////////////////////////////////////////////////////////////////////////
    WbNodeRef robot = wb_supervisor_node_get_from_def("EPUCK_1");
    const double *position = wb_supervisor_node_get_position(robot);
     
    // Corridor
    double start_position = 0.1;
    double end_position = -0.6; 
    ...
    */
    ...
    // Wrong turn >> penalty
    if (position[0] < 0.70 && position[0] > 0.30){
        penalty = true;
    } 
    ...
    {      
      ...
      if(position[0] < 0.5) fitnessValue *= 0.6; // 2/3      
    }
