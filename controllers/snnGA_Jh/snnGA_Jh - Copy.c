...
void sense_compute_and_actuate(WbDeviceTag left_motor, WbDeviceTag right_motor) {
    ...
    for (i = 0; i < NUM_SENSORS; i++){        
        ...
        sensor_values[i] = sensor_values[i]/2000;
        //printf(">> %d : %f \n", i, sensor_values[i]);
        //fflush(stdout);
    }
    ...
    int demo = 0;
    FILE *contact;
    contact=fopen("C:\\Users\\Jhielson\\Downloads\\Code\\Code\\webots\\controllers\\rnnGAsupervisor_Jh\\demo.txt","r");
    if(contact!=NULL){
        char line[10];
        fgets( line,10,contact );
        sscanf(line,"%d",&demo);
    }
    fclose(contact);
    // Read file
    contact=fopen("C:\\Users\\Jhielson\\Downloads\\Code\\Code\\webots\\controllers\\rnnGAsupervisor_Jh\\phase.txt","r");
    if(contact!=NULL){
        char line[10];
        fgets( line,10,contact );
        sscanf(line,"%d",&phase);
    ...
    // Phase 1 only
    if(position[2] < -0.35 && phase == 1 && demo == 0){ 
        flagPosition = 1;
    } 
    
    // Wrong turn >> penalty
    if (position[0] < 0.75 && position[0] > 0.25){
            flagPosition = 1;
            penalty = true;
    }
    ...
    // Clip e-puck max speed values to avoid warning
